...
def calculate_quarterly_averages(data) -> dict:
    ...
    grades_raw = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    ...
    for entry in data:
        ...
        if quarter is None:
            continue
        grades_raw[name][subject][quarter].append(grade)
    report = {}

    # pprint(grades_raw)

    for name, subjects in grades_raw.items():
        report[name] = {}
        for subject, quarters in subjects.items():
            report[name][subject] = {}
            for quarter, grades_list in quarters.items():
                average = round(sum(grades_list) / len(grades_list), 2)
                report[name][subject][quarter] = average
