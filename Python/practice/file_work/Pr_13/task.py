...
def process_sales_data(file_path, output_dir):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            name, data, amount, category, city = line.strip().split(',')
            year = data[:4]
            month = data[5:7]
            amount = int(amount)
        ...
        for year, month, in category_totals:
            month_dir = os.path.join(output_dir, year, month)
            os.makedirs(month_dir, exist_ok=True)
            report_path = os.path.join(month_dir, 'report.txt')

            with open(report_path, 'w', encoding='utf-8') as report_file:
                total_sum = 0
                for category, total in sorted(category_totals[(year, month)].items()):
                    report_file.write(f"{category},{total}\n")
                    total_sum += total
                report_file.write(f"Full,{total_sum}\n")

        for year, month, category in grouped_sales:
            month_dir = os.path.join(output_dir, year, month)
            os.makedirs(month_dir, exist_ok=True)
            category_path = os.path.join(month_dir, f'{category}.txt')
            sales = sorted(grouped_sales[(year, month, category)], key=lambda x: x['data'])

            with open(category_path, 'w', encoding='utf-8') as report_file:
                for sale in sales:
                    report_file.write(f"{sale['data']},{sale['name']},{sale['amount']}\n")
